...
import { GENERATOR_VERSION } from '@ottie/scenario-core';
...
import {
  ...
  createStarterGenerator,
  generatePackageWorlds,
  generationRequest,
  loadStarterClosure,
  loadStarterPackages,
} from '@ottie/starter-content';
...
const createRuntimeGenerator = () => createStarterGenerator(RESOLVER);
const GENERATED = SCENARIOS.flatMap((pkg) => generatePackageWorlds(pkg, createRuntimeGenerator()));
...
const registryContext = { assets: RESOLVER, sources: [...STARTER.sources, ...CLOSURE.sources] };
...
describe('T1 starter scenario packages: identity and pins', () => {
  ...
  it('pins match the shared generator, world schema, source profile and the runtime closure registry hash', () => {
    for (const pkg of SCENARIOS) {
      ...
      expect(pkg.pins.assetRegistryHash).toBe(CLOSURE.registryHash);
  ...
  it('runtime dimensions of placed signs and markings match the A1 source millimetres converted to metres', () => {
    for (const { pkg, authored, world } of GENERATED) {
      for (const candidate of pkg.assetCandidates) {
        ...
        if (!face) continue;
        const resolved = RESOLVER.resolve(face.asset);
        expect(resolved.ok, face.asset.id).toBe(true);
        const dims = resolved.ok && resolved.asset.geometry.kind === 'face' ? resolved.asset.geometry.face : null;
...
describe('T1 starter scenario packages: comparisons stay separate from their base worlds', () => {
  ...
  it('the arrow-dark comparison hands the right turn back to the circular green (permissive), proving the change is only the arrow', () => {
    ...
    const request = comparisonGenerationRequest(pkg, c);
    const result = createRuntimeGenerator().generate(request ?? (() => { throw new Error('no request'); })());
