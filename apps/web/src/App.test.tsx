...
describe('web shell (F0 smoke)', () => {
  ...
  it('switches fixtures and camera presets without a WebGL context', async () => {
    ...
    render(<App />);
    expect(screen.getByTestId('world-summary')).toHaveTextContent('development_fixture');
  ...
  it('lists every downstream module with its owner', () => {
    render(<App />);
    const status = screen.getByTestId('module-status');
